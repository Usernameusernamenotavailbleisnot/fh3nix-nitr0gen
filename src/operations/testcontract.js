...
class ContractTesterManager {
    ...
    async performParameterVariationTests(contractAddress, abi) {
        try {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                await addRandomDelay(this.configManager.getDelayConfig(), this.walletNum, `parameter test ${i+1}/${iterations}`);
    ...
    async performStressTests(contractAddress, abi) {
        try {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                // Add random delay before test
                await addRandomDelay(this.configManager.getDelayConfig(), this.walletNum, `stress test ${i+1}/${iterations}`);
    ...
    async performBoundaryTests(contractAddress, abi) {
        try {
            ...
            for (let i = 0; i < boundaryValues.length; i++) {
                ...
                // Add random delay before test
                await addRandomDelay(this.configManager.getDelayConfig(), this.walletNum, `boundary test ${i+1}/${boundaryValues.length}`);
    ...
    async executeContractTestingOperations() {
        if (!this.configManager.isEnabled('contract_testing')) {
            this.logger.warn(`Contract testing operations disabled in config`);
            return true;
        }
        
        this.logger.header(`Starting contract testing operations...`);
        ...
        try {
            ...
            await addRandomDelay(this.configManager.getDelayConfig(), this.walletNum, "test contract deployment");
            ...
            const testSequences = this.configManager.get(
